...
### Basic Usage
```yaml
- uses: fireboomio/r2-upload-action@latest
  with:
...
### Custom Usage
```yaml
- uses: fireboomio/r2-upload-action@latest # Can be any release
  with:
    r2-account-id: ${{ secrets.R2_ACCOUNT_ID }}
    r2-access-key-id: ${{ secrets.R2_ACCESS_KEY_ID }}
    r2-secret-access-key: ${{ secrets.R2_SECRET_ACCESS_KEY }}
...
```
See the latest [action.yml](https://github.com/fireboomio/r2-upload-action/blob/master/action.yml) for every input and output or take a look below.
<!-- -->
...
See the [contributing guide](https://github.com/fireboomio/r2-upload-action/blob/master/CONTRIBUTING.md) for more detail on how to implement something into the project.